...
public class HotelService {
    ...
    public List<HotelResponse> list(String cidade) {
        return hotels.findAllByOrderByNome().stream()
            .filter(h -> cidade == null || cidade.isBlank() || CityNames.same(h.getCidade(), cidade))
    ...
    public HotelResponse update(UUID id, HotelRequest request) { return save(lock(id), request); }
    ...
    private Hotel lock(UUID id) { return hotels.findLockedById(id).orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Hotel não encontrado")); }
}
